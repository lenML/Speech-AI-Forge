...
from modules.repos_static.index_tts.indextts.gpt.model_v2 import UnifiedVoice
from modules.repos_static.index_tts.indextts.utils.maskgct_utils import (
    build_semantic_codec,
)
from modules.repos_static.index_tts.indextts.utils.checkpoint import load_checkpoint
from modules.repos_static.index_tts.indextts.utils.front import (
    TextNormalizer,
    TextTokenizer,
)

from modules.repos_static.index_tts.indextts.s2mel.modules.commons import (
    load_checkpoint2,
    MyModel,
)
from modules.repos_static.index_tts.indextts.s2mel.modules.bigvgan import bigvgan
from modules.repos_static.index_tts.indextts.s2mel.modules.campplus.DTDNN import (
    CAMPPlus,
)
from modules.repos_static.index_tts.indextts.s2mel.modules.audio import mel_spectrogram
...
def build_semantic_model(path_="./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt"):
    ...
    return semantic_mean, semantic_std
...
class IndexTTS2:
    def __init__(
        ...
        use_deepspeed=False,
        # +++
        extract_features_model: SeamlessM4TFeatureExtractor = None,
        semantic_model: Wav2Vec2BertModel = None,
        semantic_code_ckpt_path: str = None,
        campplus_ckpt_path: str = None,
        # 这个本来是根据配置来的，但是根据配置的话不太好接入 models 文件夹，所以改为传入
        # NOTE: 由此，如果后续模型升级，可能需要关注这个配置问题
        bigvgan_model: bigvgan.BigVGAN = None,
    ):
        """
        Args:
        ...
        ), "extract_features_model type wrong"
        assert semantic_code_ckpt_path, "semantic_code_ckpt_path is None"
        assert campplus_ckpt_path, "campplus_ckpt_path is None"
        ...
        if self.use_cuda_kernel:
            ...
            try:
                from modules.repos_static.index_tts.indextts.BigVGAN.alias_free_activation.cuda import (
                    load,
                )
        ...
        # self.extract_features: SeamlessM4TFeatureExtractor = SeamlessM4TFeatureExtractor.from_pretrained(
        #     "facebook/w2v-bert-2.0"
        # )
        self.extract_features = extract_features_model
        self.semantic_mean, self.semantic_std = build_semantic_model(
        ...
        # )
        semantic_code_ckpt = semantic_code_ckpt_path
