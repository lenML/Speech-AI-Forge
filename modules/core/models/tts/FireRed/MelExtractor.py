...
class MelSpectrogramExtractor:
    ...
    def from_array(self, wav_data: np.ndarray, wav_sr: int) -> torch.Tensor:
        """
        Process the audio data from a numpy array and generate mel spectrogram.

        Args:
            wav_data (np.ndarray): Input audio waveform (float32).
            wav_sr (int): The sampling rate of the input audio.

        Returns:
            np.ndarray: Mel spectrogram of the input audio.
        """
        ...
        mel = mel.squeeze(0).transpose(1, 0)  # (t, num_mels)
        return mel
