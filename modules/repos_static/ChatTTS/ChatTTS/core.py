...
from .model import DVAE, GPT, Tokenizer, gen_logits
...
class Chat:
    ...
    def _load(
        ...
        if device is None:
            ...
            self.logger.log(logging.INFO, f"use {device} with {dtype}")
        self.device = device
        self.dtype = dtype
        ...
        vocos.load_state_dict(
            torch.load(
                vocos_ckpt_path, weights_only=True, mmap=True, map_location=map_location
            )
        )
        self.vocos = vocos
        ...
        dvae.load_state_dict(
            torch.load(
                dvae_ckpt_path, weights_only=True, mmap=True, map_location=map_location
            )
        )
        self.dvae = dvae
    ...
    def _infer_code(
        ...
        if params.spk_emb is not None:
            text = [
                f"{params.prompt1}[Stts]{params.prompt2}[spk_emb]{txt_smp}{i}[Ptts]{params.prefix}"
                for i in text
            ]
        else:
            text = [
                f"{params.prompt1}[Stts]{params.prompt2}[empty_spk]{txt_smp}{i}[Ptts]{params.prefix}"
                for i in text
            ]
