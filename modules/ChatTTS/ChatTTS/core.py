...
class Chat:
    ...
    def _load(
        ...
        if vocos_config_path:
            vocos = (
                ...
                .to(
                    ...
                    device="cpu" if "mps" in str(device) else device,
                    dtype=dtype,
                )
                .eval()
            )
        ...
        if dvae_config_path:
            ...
            dvae = DVAE(**cfg, coef=coef).to(device=device, dtype=dtype).eval()
            coef = str(dvae)
        ...
        if gpt_config_path:
            ...
            self.pretrain_models["spk_stat"] = torch.load(
                spk_stat_path, weights_only=True, mmap=True, map_location=map_location
            ).to(device=device, dtype=dtype)
            self.logger.log(logging.INFO, "gpt loaded.")

        if decoder_config_path:
            cfg = OmegaConf.load(decoder_config_path)
            decoder = DVAE(**cfg, coef=coef).to(device=device, dtype=dtype).eval()
            coef = str(decoder)
        ...
        if tokenizer_path:
            tokenizer = torch.load(tokenizer_path, map_location=map_location, mmap=True)
    ...
    def _apply_spk_emb(
        ...
        spk_emb = (
            ...
            else torch.from_numpy(self._decode_spk_emb(spk_emb)).unsqueeze(0)
        )
        n = (
            F.normalize(
                spk_emb.expand(text_len, -1),
                p=2.0,
                dim=1,
                eps=1e-12,
            )
    ...
    def _infer_code(
        ...
        if params.spk_emb is not None:
            text = [
                f"{params.prompt1}[Stts]{params.prompt2}[spk_emb]{i}[Ptts]{params.prefix}"
                for i in text
            ]
        else:
            text = [
                f"{params.prompt1}[Stts]{params.prompt2}[empty_spk]{i}[Ptts]{params.prefix}"
                for i in text
            ]
